...
int check_Left_tree(const binary_tree_t *node, int max)
{
	...
	else
	{
		if (node->n >= max)
			return (0);
		left = check_Left_tree(node->left, max);
		right = check_Left_tree(node->right, max);
...
/**
 * check_Right_tree - check if all the nodes are bigger than the
 * root specified
 * @node: node in the tree to verify condition
 * @min: value to compare
 * Return: 1 if all is bigger or equal or 0 if not
 */
int check_Right_tree(const binary_tree_t *node, int min)
{
	int left = 0, right = 0;
	...
	else
	{
		if (node->n <= min)
			return (0);
		left = check_Right_tree(node->left, min);
		right = check_Right_tree(node->right, min);
		if (left == right && left == 1)
			return (1);
		return (0);
	}
...
int binary_tree_is_bst(const binary_tree_t *tree)
{
	...
	if (tree->left && tree->left->n < tree->n)
	{
		sam = check_Left_tree(tree->left, tree->n);
		if (sam == 0)
			return (0);
	...
	if (tree->right && tree->right->n > tree->n)
	{
		sam = check_Right_tree(tree->right, tree->n);
		if (sam == 0)
			return (0);
